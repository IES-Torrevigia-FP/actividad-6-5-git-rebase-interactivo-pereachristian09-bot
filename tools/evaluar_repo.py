...
def analyze_rebase_i_activity(root: Path) -> dict:
    cp1_path = root / 'cp1-historial-original.txt'
    cp2_path = root / 'cp2-reword.txt'
    cp3_path = root / 'cp3-squash.txt'
    cp4_path = root / 'cp4-reordenar.txt'
    cp5_path = root / 'cp5-abortar-rebase.txt'
    app_path = root / 'app.md'
    ...
    cp1_history_ok = ('git log --oneline' in cp1) or ('commit 1: versión inicial' in cp1) or ('commit 1: version inicial' in cp1) or ('wip 1' in cp1)
    reword_ok = ('reword' in cp2) or ('qué hace reword' in cp2) or ('que hace reword' in cp2)
    squash_ok = ('squash' in cp3) or ('agrupar commits' in cp3) or ('agrupa cambios' in cp3)
    reorder_ok = ('reorden' in cp4) or ('orden original' in cp4) or ('orden de commits' in cp4)
    abort_ok = ('git rebase --abort' in cp5) or ('rebase --abort' in cp5)

    app_min_content_ok = ('# app' in app) and ('versión inicial' in app or 'version inicial' in app)

    return {
        'cp1_historial': {
            'ok': cp1_history_ok,
            'source': 'cp1-historial-original.txt' if cp1_path.exists() else 'missing',
        },
        'reword_checkpoint': {
            'ok': reword_ok,
            'source': 'cp2-reword.txt' if cp2_path.exists() else 'missing',
        },
        'squash_checkpoint': {
            'ok': squash_ok,
            'source': 'cp3-squash.txt' if cp3_path.exists() else 'missing',
        },
        'reorder_checkpoint': {
            'ok': reorder_ok,
            'source': 'cp4-reordenar.txt' if cp4_path.exists() else 'missing',
        },
        'abort_checkpoint': {
            'ok': abort_ok,
            'source': 'cp5-abortar-rebase.txt' if cp5_path.exists() else 'missing',
        },
        'app_final': {
...
def main():
    ...
    ap.add_argument('--required', default='app.md,cp1-historial-original.txt,cp2-reword.txt,cp3-squash.txt,cp4-reordenar.txt,cp5-abortar-rebase.txt,reflexion-6-5.md')
    ...
    rebase_i_info = analyze_rebase_i_activity(root)
    ...
    reflex_path = root / 'reflexion-6-5.md'
    ...
    evidencia_candidates = [
        'cp1', 'cp2', 'cp3', 'cp4', 'cp5',
        'reword', 'squash', 'reorden', 'abort', 'rebase', 'reflexion',
    ]
    ...
    checks_rebase_i = [
        rebase_i_info['cp1_historial']['ok'],
        rebase_i_info['reword_checkpoint']['ok'],
        rebase_i_info['squash_checkpoint']['ok'],
        rebase_i_info['reorder_checkpoint']['ok'],
        rebase_i_info['abort_checkpoint']['ok'],
    ]
    tech_hits = sum(1 for x in checks_rebase_i if x)
    ...
    total = s_git_rebase_i + s_evid + s_reflex
    ...
    resumen = {
        ...
        'scores': {
            'git_rebase_interactivo (max_4)': s_git_rebase_i,
    ...
    with (outdir / 'metricas.csv').open('w', newline='', encoding='utf-8') as f:
        ...
        w.writerow(['git_rebase_interactivo (max 4)', s_git_rebase_i])
    ...
    md.append('|---|---:|')
    md.append(f"| Uso de git rebase -i (reword, squash, reordenacion, abort) | {s_git_rebase_i}/4 |")
    md.append(f"| Evidencias (checkpoints + archivos) | {s_evid}/3 |")
    md.append(f"| Reflexion 6.5 | {s_reflex}/3 |")
    ...
    md.append('## Checks de rebase interactivo')
    md.append(f"- Historial original (cp1): {'si' if rebase_i_info['cp1_historial']['ok'] else 'no'}  ")
    md.append(f"- Evidencia de reword (cp2): {'si' if rebase_i_info['reword_checkpoint']['ok'] else 'no'}  ")
    md.append(f"- Evidencia de squash (cp3): {'si' if rebase_i_info['squash_checkpoint']['ok'] else 'no'}  ")
    md.append(f"- Evidencia de reordenacion (cp4): {'si' if rebase_i_info['reorder_checkpoint']['ok'] else 'no'}  ")
    md.append(f"- Evidencia de abort (cp5): {'si' if rebase_i_info['abort_checkpoint']['ok'] else 'no'}  ")
    md.append(f"- app.md final presente y minimo valido: {'si' if rebase_i_info['app_final']['exists'] and rebase_i_info['app_final']['minimum_content_ok'] else 'no'}  ")
    md.append('')
